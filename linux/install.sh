...
# ============================================================================
# Script di Installazione - Download Organizer
# ============================================================================
# Questo script installa e configura l'organizzatore automatico di Download
# ============================================================================

echo "╔════════════════════════════════════════════════════════════╗"
echo "║  Installazione Download Organizer                          ║"
echo "╚════════════════════════════════════════════════════════════╝"
echo ""
...
if [[ ":$PATH:" != *":$INSTALL_DIR:"* ]]; then
    echo "⚠️  $INSTALL_DIR non è nel PATH"
...
# Rileva se esiste una struttura con nomi di un'altra lingua
DOWNLOAD_DIR_CHECK="$HOME/Download"
...
if [ -d "$DOWNLOAD_DIR_CHECK" ]; then
    ...
    # Controlla le lingue conosciute per vedere se ci sono cartelle vecchie
    for old_lang in it en es fr de pt; do
        if [ "$old_lang" = "$LANGUAGE" ]; then
            continue
        fi

        # Carica temporaneamente la vecchia lingua
...
echo ""
echo "    Questo eseguirà lo script:"
...
echo ""
echo "╔════════════════════════════════════════════════════════════╗"
echo "║  Alternative di Frequenza                                  ║"
echo "╚════════════════════════════════════════════════════════════╝"
echo ""
echo "Ogni ora:        0 * * * * $SCRIPT_PATH"
echo "Ogni 2 ore:      0 */2 * * * $SCRIPT_PATH"
echo "Ogni 6 ore:      0 */6 * * * $SCRIPT_PATH"
echo "Ogni giorno:     0 0 * * * $SCRIPT_PATH"
echo "Al login:        @reboot $SCRIPT_PATH"
echo ""
